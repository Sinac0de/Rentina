  ...
  return (
    <div className="flex flex-col w-full h-fit gap-2 my-5 lg:my-0">
      ...
      <select
        id={id}
        name={id}
        className="w-full p-4 bg-[#F6F7F9] rounded-[10px] text-xs text-secondary-300 focus:ring-1 focus:ring-secondary-300 border-r-[14px] border-transparent px-4 outline-none lg:text-sm cursor-pointer"
      >
        {options.map((option) => {
          return (
            <option
